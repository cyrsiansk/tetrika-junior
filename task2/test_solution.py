...
def test_collect_file_creates_output_file(url_type, title_text, tmp_path):
    ...
    try:
        path = collect_file(url_type, ALPHABET, parse_args, max_workers=5, path=str(output_file))
...
def test_invalid_url_fails(tmp_path):
    parse_args = ABCTypeParseArgs(
    ...
    with pytest.raises(Exception):
        collect_file(broken_url, ALPHABET, parse_args, max_workers=3, path=tmp_path / "invalid.csv")
